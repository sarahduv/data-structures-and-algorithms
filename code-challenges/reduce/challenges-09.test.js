...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let countOfItems = 0;
  arr.reduce(x => countOfItems++, 'whatever');
  return countOfItems;
};
...
const reversedString = (str) => {
  ...
  let arr = str.split('');
  let revArr = [];
  arr.reduce((resultSoFar, x) => revArr.unshift(x), 'whatever');
  return revArr.join('');
};
...
const calculateAverage = (arr) => {
  ...
  const result = arr.reduce(
    (resultSoFar, num) => {return {sum: resultSoFar.sum + num, count: resultSoFar.count + 1}},
    {sum: 0, count: 0}
  );
