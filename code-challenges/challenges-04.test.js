...
const isCapitalized = (str) => {
  // Solution code here...
  const result = str.match(/[A-Z]\w+/gm);
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let arrThree = [];
  const regexThree = /^[A-J]\w+/gmi;
  arr.forEach(element => {
    let three = element.match(regexThree);
    if (three !== null){
      arrThree = arrThree.concat(three);
    }
  })
  return arrThree;
};
...
const matchMonth = (input) => {
  // Solution code here...
  const regexFour = /^[oO]ct(ober)?$/gm;
  return regexFour.test(input);
};
...
const noPunctuation = str => {
  // Solution code here...
  const regexFive = /\w+\s/gmi;
  return str.match(regexFive);
};
...
const findShells = (str) => {
  // Solution code here...
  return str.match(/(sea)?(sh)?(s)?ells/gmi);
};
