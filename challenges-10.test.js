...
const count = (target, input) => {
  // Solution code here...
  const result = input.map(arr => arr.filter(num => num === target))
    .map(arr => arr.length).reduce((a, b) => a+b, 0);
  return result;
  // [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]]
  // [[5][5,5,5,],[]]
  // [1,3,0]
  // 4
};
...
const totalSum = (input) => {
  // Solution code here...
  const result = input.map(arr => arr.reduce((a, b) => a+b, 0)).reduce((a,b) => a+b, 0);
  return result;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const result = input.map(arr => arr.filter(num => typeof(num)==='number' && num%5 === 0).map(num => Math.pow(2, num)));
  return result;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const result = data.filter(character => character.gender === 'female' || character.gender === 'male')
...
let findShortest = (data) => {
  // Solution code here...
  const result = data.reduce((a,b) => a.name.length < b.name.length ? a : b).name;
  return result;
};
